...
class MeditationController extends Controller
{
    ...
    {
        $meditations = Meditation::where('tutor_id',Auth::user()->id)->get();
        // dd($Meditations);
        return view('tutor.meditation.index',compact('meditations'));
    }

    public function show($id)
    {
        $meditation = Meditation::findorfail($id);        
        return view('tutor.meditation.show',compact('meditation'));
    }
    ...
    public function create()
    {
        ...
        foreach($studentusers as $studentuser){
            $payment = $studentuser->student->payments->where('end_date', '>', Carbon::now()->toDateTimeString())->first();
            if($payment){
                $payments[] = $payment->load('user', 'domains');
            }
        }

        // dd($students);
        return view('tutor.meditation.create',compact('payments'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $meditation->payment_id = $request->payment_id;        
        $meditation->save();
    ...
    public function edit($id)
    {
        ...
        return view('tutor.meditation.edit',compact('meditation','studentusers'));
    }
    ...
    public function calendar(Request $request)
    {
            ...
            foreach ($meditations as $key => $meditation) {
                foreach($meditation->sessions as $session){
                    $events[] = Calendar::event(
                        ucwords($meditation->student->name),
                        false,
                        ...
                        [
                            'color' => '#3ae2ea',
                            'url' => route('tutor.session.index').'/'.$session->id,
                        ]
                    );
        ...
        return view('tutor.meditation.calendar', compact('calendar','latest'));
    }
